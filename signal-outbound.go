...
func sendSignalMessage(cfg *Config, data []byte) error {
	...
	if err != nil {
		log.Printf("Error sending message to signal-cli SSE: %v", err)
		return fmt.Errorf("sending message to signal-cli SSE: %w", err)
	}
	defer func() {
...
func sendSignalMessageService(ctx context.Context, nc *nats.Conn, cfg *Config) {
	...
	_, err := nc.QueueSubscribe(
		...
		cfg.NatsQueueGroup,
		func(msg *nats.Msg) {
			log.Printf("Processing message from topic '%s'...", msg.Subject)
	...
	if err != nil {
		log.Fatalf("Error subscribing to topic '%s': %v", cfg.NatsSubjectOut, err)
	}

	log.Printf("Successfully subscribed to NATS topic '%s' in queue group '%s'.", cfg.NatsSubjectOut, cfg.NatsQueueGroup)

	<-ctx.Done() // Wait for context cancellation to signal shutdown
	log.Println("Shutting down NATS Core subscription.")
}
